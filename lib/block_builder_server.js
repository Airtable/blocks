...
class BlockBundleServer {
    ...
    _setUpBlockSdkAndWrapper() {
        ...
        if (!nodeModulesDirExists) {
            console.log('Must run npm install first');
            process.exit(1);
        }
        // Get the block entry point
        let frontendEntryModulePath;
        try {
            const blockMetadataJson = fs.readFileSync(path.join(blockDirPath, '.block'));
    ...
    start() {
        ...
        // The user might have made changes since the last time the server was
        // running, so bundle here to ensure we are serving the latest package
        this._bundle();
    }
    _bundle() {
