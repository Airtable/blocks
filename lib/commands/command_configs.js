...
const commandConfigs = {
    [Commands.CLONE]: {
        ...
        validateConfig: config => {
            const blockIdentifier = String(config.blockIdentifier);
            const blockIdentifierSplit = blockIdentifier.split('/');
            if (!blockIdentifierSplit[0] || !blockIdentifierSplit[1]) {
                return {pass: false, reason: 'Block identifier must be of format <applicationId>/<blockId>'};
            }
            return {pass: true};
        },
        argDescriptions: {
        ...
        runCommandAsync: async config => {
            ...
            const blockIdentifier = String(config.blockIdentifier);
            const [appId, blockId] = blockIdentifier.split('/');

            const blockCloneAsync = require('./clone');
            await blockCloneAsync(
                environment,
                appId,
                blockId,
                config.blockDirPath,
