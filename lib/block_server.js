...
class BlockServer {
    ...
    _setUpBackendProcess() {
        ...
        this._transpileBackendCode();
    }
    _transpileBackendCode() {
        const blockDirPath = process.cwd();
        const backendDir = path.join(blockDirPath, 'backendRoute');
        const transpiledDir = path.join(blockDirPath, blocksConfigSettings.TRANSPILED_BACKEND_DIR);
        if (!fs.existsSync(transpiledDir)) {
            fs.mkdirSync(transpiledDir);
        }
        const babelConfig = {
        ...
        for (const backendModule of backendModules) {
            const name = backendModule.metadata.name;
            const codePath = path.join(backendDir, name + '.js');
            const code = fs.readFileSync(codePath, 'utf8');
            const transpiledCodePath = path.join(transpiledDir, name + '.js');
            const transpiledCode = Babel.transform(code, babelConfig).code;
            fs.writeFileSync(transpiledCodePath, transpiledCode);
        }
    }
    _setUpExpressRoutes() {
        const blockDirPath = process.cwd();
