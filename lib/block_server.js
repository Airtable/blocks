...
class BlockServer {
    constructor({transpileAll = false} = {}) {
        ...
        this._watchBackendCodeIfNeeded();
    ...
    _watchBackendCodeIfNeeded() {
        const blockDirPath = process.cwd();
        // Restart the backend process when code changes.
        if (this.doesBlockHaveBackend()) {
            fs.watchFile(path.join(blockDirPath, blocksConfigSettings.BLOCK_FILE_NAME), this.startBackendProcess.bind(this));
            ...
            for (const folderName of foldersToWatch) {
                ...
                if (fs.existsSync(folderPath)) {
                    fs.watch(folderPath, this.startBackendProcess.bind(this));
                }
            }
        }
    }
