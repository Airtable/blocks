...
const findBlockDirPath = require('./find_block_dir_path');
...
class BlockServer {
    constructor({transpileAll = false} = {}) {
        this._blockDirPath = findBlockDirPath();
    ...
    _watchBackendCode() {
        const blockJsonPath = path.join(this._blockDirPath, blocksConfigSettings.BLOCK_FILE_NAME);
        const backendRoutePath = path.join(this._blockDirPath, 'backendRoute');
        const sharedPath = path.join(this._blockDirPath, 'shared');
    ...
    _setUpExpressRoutes() {
        ...
        this._expressApp.use(bodyParser.json({limit: '5mb'}));
        ...
        this._expressApp.get('/__runFrame/bundle.js', (req, res) => {
            res.sendFile(blocksConfigSettings.BUNDLE_FILE_NAME, {
                root: path.join(this._blockDirPath, blocksConfigSettings.BUILD_DIR),
            });
        });

        this._expressApp.options('/__runFrame/registerBlockInstallationMetadata', (req, res) => {
            res.set({
        ...
        this._expressApp.post('/__runFrame/registerBlockInstallationMetadata', (req, res) => {
            if (!req.body || !req.body.applicationId || !req.body.blockInstallationId) {
                ...
                const apiKey = getApiKeySync(this._blockDirPath);
                const blockData = JSON.parse(fs.readFileSync(path.join(this._blockDirPath, 'block.json')));
    ...
    _fireRequestToBackend(req, res) {
        ...
        this._apiClient.fetchAccessPolicyAsync().then(body => {
            ...
            const event = {
                ...
                _applicationId: this._apiClient.applicationId,
                _blockInstallationId: this._apiClient.blockInstallationId,
            };
    ...
    _setUpBlockSdkAndWrapper(url) {
        ...
        const packageJson = fs.readFileSync(path.join(this._blockDirPath, 'package.json'));
        ...
        const nodeModulesDirPath = path.join(this._blockDirPath, 'node_modules');
        const nodeModulesDirExists = fs.existsSync(nodeModulesDirPath);
        if (!nodeModulesDirExists) {
            console.log(`Must run yarn in ${this._blockDirPath} first to install modules`);
            process.exit(1);
        }
        ...
        try {
            const blockMetadataJson = fs.readFileSync(
                path.join(this._blockDirPath, blocksConfigSettings.BLOCK_FILE_NAME),
            );
        ...
        const buildDirPath = path.join(this._blockDirPath, blocksConfigSettings.BUILD_DIR);
    ...
    _doesBlockHaveBackend() {
        const backendDirPath = path.join(this._blockDirPath, 'backendRoute');
