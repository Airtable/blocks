...
class BlockServer {
    ...
    _watchBackendCode() {
        ...
        const foldersToWatch = ['backendRoute', 'shared'];
        for (const folderName of foldersToWatch) {
            const folderPath = path.join(blockDirPath, folderName);
            if (fs.existsSync(folderPath)) {
                fs.watch(folderPath, this._setUpBackendProcess.bind(this));
            }
        }
    }
    _setUpBackendProcess() {
        ...
        this._backendProcess = childProcess.fork(path.join(__dirname, 'block_backend_handler'), {
            stdio: 'pipe',
            env: {
                // Chalk doesn't think this separate process can support colors, and it doesn't know
                // its output is being piped back to this process, so it won't apply any styles.
                // Setting this env variable ensures Chalk will apply styles no matter what.
                FORCE_COLOR: 1,
            },
        });
