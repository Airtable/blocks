...
class BlockBundleServer {
    ...
    _setUpBlockSdkAndWrapper() {
        ...
        if (!_.includes(dependenciesList, 'react') || !_.includes(dependenciesList, 'react-dom')) {
            console.log('Please ensure that react and react-dom packages are installed and listed in package.json');
            process.exit(1);
        }
        ...
        if (!frontendEntryModuleExists) {
            console.log(`The entry module at ${frontendEntryModulePath} does not exist. If you changed the entry module, please run 'block rename-entry <newName>'`);
            process.exit(1);
        }
    ...
    setPublicUrlForLongPoll(basePublicUrl) {
        ...
        this._bundler.transform(envify({
    ...
    startLocalAsync(port) {
        return Promise
            ...
            .then(([key, cert]) => new Promise((resolve, reject) => {
                const server = https.createServer({cert, key}, this._expressApp);
                server.listen(port).on('error', reject).on('listening', resolve);
            }));
    }
