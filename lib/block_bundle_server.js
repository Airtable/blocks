...
class BlockBundleServer {
    ...
    _setUpExpressRoutes() {
        ...
        this._expressApp.get('/longPoll', (req, res, next) => {
            // Let's use the random query param as the request id
            const requestId = req.query.random;
            if (!requestId) {
                throw new Error('Must provide the random query param');
            }
            ...
            this._pendingLongPollResponsesByRequestId[requestId] = res;
        });
    ...
    _setUpBlockSdkAndWrapper() {
        ...
        // TODO: Figure out what to do if the developer renamed the entry module
