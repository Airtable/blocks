...
class BlockBundleServer {
    constructor({transpileAll = false} = {}) {
        ...
        this._restartingBackendProcess = false;
        // Watch for changes to backend code.
        fs.watchFile(path.join(blockDirPath, blocksConfigSettings.BLOCK_FILE_NAME), this._onBackendFileChanges.bind(this));
        fs.watch(path.join(blockDirPath, 'backendRoute'), this._onBackendFileChanges.bind(this));
    ...
    _onBackendFileChanges() {
        // Restart the backend process when code changes.
        console.log('Updating backend...');
        this._restartingBackendProcess = true;
    ...
    _setUpExpressRoutes() {
        ...
        this._expressApp.use((req, res) => {
            if (this._restartingBackendProcess) {
                // If the backend process is restarting, return
                // a 503 to all requests during this period.
                res.status(503).send({
