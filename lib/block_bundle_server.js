...
class BlockBundleServer {
    ...
    _setUpExpressRoutes() {
        ...
        // Any request other than for the bundle or the backend routes should 404.
        this._expressApp.use((req, res, next) => {
    ...
    _setUpBackendRoutes() {
        ...
        for (const backendModule of backendModules) {
            const {name, urlPath, method} = backendModule.metadata;

            const routeFunction = require(path.join(blockDirPath, 'backendRoute', name));
            const wrappedRouteFunction = (req, res) => {
                const responsePromise = Promise.resolve(routeFunction(req));
            ...
            if (method === 'get') {
                this._expressApp.get(urlPath, wrappedRouteFunction);
            } else if (method === 'post') {
                this._expressApp.post(urlPath, wrappedRouteFunction);
            } else if (method === 'put') {
                this._expressApp.put(urlPath, wrappedRouteFunction);
            } else if (method === 'patch') {
                this._expressApp.patch(urlPath, wrappedRouteFunction);
            } else if (method === 'delete') {
                this._expressApp.delete(urlPath, wrappedRouteFunction);
            }
        }
    }
    ...
    setPublicBaseUrl(publicBaseUrl) {
        this._setUpBlockSdkAndWrapper(publicBaseUrl);
        ...
        this._bundler.transform(
            envify({
                BLOCK_LONG_POLL_URL: `${publicBaseUrl}/__runFrame/poll`,
            }),
        );
    }
    startAsync(port) {
