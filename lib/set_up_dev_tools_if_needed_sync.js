...
module.exports = function setUpEslintIfNeededSync(blockDirPath) {
    ...
    _.each(devDependencies, (version, name) => {
        ...
        if (!currentVersion) {
            ...
            if (semver.gte(currentVersion, cleanVersion)) {
                console.log(
                    `  * ${name}: skip ${currentVersion} -> ${cleanVersion}`,
                );
            } else {
                console.log(
                    `  * ${name}: upgrade ${currentVersion} -> ${cleanVersion}`,
                );
                packageJson.devDependencies[name] = version;
            }
        }
    ...
    const copyConfigFile = name => {
        ...
        if (!blockVersion) {
            ...
            console.log(
                `  * ${name}: exists, but different to official version. skipping`,
            );
        }
    };
