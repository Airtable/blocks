...
module.exports = function blockFormat(blockDirPath) {
    ...
    const files = [
        ...glob.sync(path.join(blockDirPath, 'frontend', '**/*.js')),
        ...glob.sync(path.join(blockDirPath, 'shared', '**/*.js')),
        ...glob.sync(path.join(blockDirPath, 'backendRoute', '**/*.js')),
    ];
    ...
    files.forEach(fileName => {
        ...
        const formattedSource = prettier.format(
            originalSource,
            Object.assign({}, config, {filepath: fileName}),
        );
