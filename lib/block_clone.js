...
module.exports = function blockCloneAsync(environment, applicationId, blockId, blockDirPath, apiKey) {
    ...
    return blockDirValidationPromise.then(() => {
        ...
        const writeBlockFilesFromApiResponsePromise = writeFilesFromApiResponseAsync(response, blockDirPath, {environment, applicationId, blockId});
        ...
        const writePackageJsonPromise = fsUtils.writeFileAsync(path.join(blockDirPath, 'package.json'), JSON.stringify({
            dependencies: packageVersionByName,
            devDependencies: devDependencies,
        }, null, 4));

        setUpEslintIfNeededSync(blockDirPath);
        ...
        const writeGitignoreFilePromise = fsUtils.writeFileAsync(path.join(blockDirPath, '.gitignore'), gitignoreContents.join('\n'));

        return Promise.all([writeBlockFilesFromApiResponsePromise, writePackageJsonPromise, writeAirtableApiKeyFilePromise, writeGitignoreFilePromise]);
    });
};
