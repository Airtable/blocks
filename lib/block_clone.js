...
module.exports = function blockCloneAsync(applicationId, blockId, apiKey, blockDirPath) {
    ...
    return blockDirValidationPromise.then(() => {
        ...
        return getRequestAsync(options).then(response => {
            ...
            return Promise.all(createSubDirsPromises).then(() => {
                ...
                const createPackageJson = writeFileAsync(path.join(blockDirPath, 'package.json'), JSON.stringify({
                    dependencies: packageVersionByName,
                }, null, 4));

                return Promise.all(_.concat(writeFilesPromises, writeBlockFilePromise, createPackageJson));
            });
        });
    });
}
