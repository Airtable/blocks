...
async function callUserCodeForEventAsync(event, routes) {
    ...
    try {
        const routeHandlerModule = require(path.join(this.blockDirPath, blocksConfigSettings.BUILD_DIR, 'backendRoute', route.metadata.name));
...
async function symlinkBackendAndSharedFoldersAsync() {
    const buildDirPath = path.join(this.blockDirPath, blocksConfigSettings.BUILD_DIR);
    const folderNames = ['backendRoute', 'shared'];
    for (const folderName of folderNames) {
        const folderPath = path.join(buildDirPath, folderName);
        const nodeModulesPath = path.join(this.blockDirPath, 'node_modules', folderName);
        fsUtils.symlinkIfNeededAsync(folderPath, nodeModulesPath);
    }
}

async function transpileFileIfNeededAsync(moduleName, moduleType, srcDirPath, destDirPath) {
...
async function transpileBackendAndSharedCodeAsync(modules) {
    ...
    for (const module of modulesToTranspile) {
        const transpilationPromise = transpileFileIfNeededAsync(module.metadata.name, module.metadata.type, this.blockDirPath, buildDirPath);
...
async function checkIfBackendRoutesExistInBlockJson(modules) {
    ...
    const backendRouteFiles = await fsUtils.readDirAsync(path.join(this.blockDirPath, 'backendRoute'));
    const jsRegex = /\.js$/;
...
async function setUpBackend() {
    this.blockDirPath = findBlockDirPath();
    // Parse block.json.
    const blockJson = await fsUtils.readFileAsync(path.join(this.blockDirPath, 'block.json'));
