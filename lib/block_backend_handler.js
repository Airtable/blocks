...
function doesPostOrPutOrDeleteRouteExistForPath(eventPath, routes) {
    const allowedMethods = ['POST', 'PUT', 'DELETE'];
    for (const routeId of Object.keys(routes)) {
        const route = routes[routeId];
        const routeMethod = route.metadata.method.toUpperCase();
        const routePath = route.metadata.urlPath;

        if (allowedMethods.includes(routeMethod)) {
            const re = pathToRegexp(routePath);
            const match = re.exec(eventPath);
            if (match) {
                return true;
            }
        }
...
async function callUserCodeForEvent(event, routes) {
    ...
    // If an OPTIONS request is sent, send the appropriate cors headers if a route matching the path exists.
    if (event.method.toUpperCase() === 'OPTIONS' && doesPostOrPutOrDeleteRouteExistForPath(event.path, routes)) {
        return {
            statusCode: 200,
            body: '',
            headers: {
