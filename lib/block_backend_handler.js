...
async function transpileFileAsync(moduleName, moduleType, srcDirPath, destDirPath) {
    ...
    // No-op if source file hasn't changed in the last minute.
    ...
    if (!srcFileStat) {
        console.warn(`File ${getFormattedProjectPath(moduleType, moduleName)} does not exist.`);
        console.warn('Please add this file or remove its entry in block.json.');
        return;
    }
    if (destFileStat && destFileStat.mtimeMs > srcFileStat.mtimeMs + slop) {
        return;
    }
