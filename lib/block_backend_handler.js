...
async function callUserCodeForEvent(event, routes) {
    ...
    try {
        const routeHandlerModule = require(path.join(blockDirPath, blocksConfigSettings.TRANSPILED_BACKEND_ROUTE_DIR, route.metadata.name));
...
function transpileBackendCode(backendModules) {
    const blockDirPath = process.cwd();
    const backendDir = path.join(blockDirPath, 'backendRoute');
    const transpiledDir = path.join(blockDirPath, blocksConfigSettings.TRANSPILED_BACKEND_ROUTE_DIR);
    if (!fs.existsSync(transpiledDir)) {
        fs.mkdirSync(transpiledDir);
    }
    const blockBabelConfig = {
...
function setUpBackend() {
    // Parse block.json and transpile all backend modules.
    const blockDirPath = process.cwd();
    ...
    transpileBackendCode(backendModules);
    // Create a routes object to store backend modules.
    const routes = {};
    for (const backendModule of backendModules) {
