...
async function transpileBackendAndSharedCodeAsync(modules) {
    ...
    for (const module of modulesToTranspile) {
        const transpilationPromise = transpileFileIfNeededAsync(module.metadata.name, module.metadata.type, blockDirPath, buildDirPath);
...
async function checkIfBackendRoutesExistInBlockJson(modules) {
    ...
    const backendRouteFiles = await fsUtils.readdirSync(path.join(blockDirPath, 'backendRoute'));
    const jsRegex = /\.js$/;
