...
import introGif from './tutorial_todo_1.gif';
import setupGif from './tutorial_todo_2.gif';
import part1Complete from './tutorial_todo_3.png';
import storingTableNameGif from './tutorial_todo_4.gif';
import permissionsGif from './tutorial_todo_5.gif';
...
<!-- TODO(evanhahn): can we do this without having to do imports and <Image>s? -->
<!--                 in other words, can we do it with ![](./image.png)?       -->
...
**We’ll assume you’ve already read [the setup guide](guides/setup).**
...
-   Part 1
    ...
    -   [useBase](api#usebase) hook to get [base](api#base) object and subscribe to schema changes.
    -   [base.getTableByName()](api#gettablebyname) to get a [table](api#table) object.
    -   [table.selectRecords()](api#selectrecords), [useRecords](api#userecords) hook, and
        [record.primaryCellValueAsString](api#primarycellvalueasstring) to read the records in a
        table.
    -   [expandRecord()](api#expandrecord) to expand records in Airtable.

-   Part 2
    -   [base.getTableByNameIfExists()](api#gettablebynameifexists) and
        [base.getTableByIdIfExists()](api#gettablebyidifexists)
    -   [TablePicker](api#tablepicker) component.
-   Part 3
    -   [globalConfig](api#globalconfig) to store block configuration.
    -   [useGlobalConfig](api#useglobalconfig) hook to watch changes to globalConfig.
    -   [TablePickerSynced](api#tablepickersynced) component.
