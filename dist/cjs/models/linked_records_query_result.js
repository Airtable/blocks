...
class LinkedRecordsQueryResult extends _query_result.default {
  ...
  constructor(record, field, opts) {
    ...
    this._poolKey = "".concat(record.id, "::").concat(field.id, "::").concat(linkedTableId); // we could rely on QueryResult's reuse pool to make sure we get back
      ...
      return _regenerator.default.wrap(function _callee2$(_context2) {
        while (1) {
          switch (_context2.prev = _context2.next) {
            case 0:
              pool.registerObjectForReuse(_this2);

              _this2._watchOrigin();

              _this2._watchLinkedQueryResult();

              _context2.next = 5;
              return Promise.all([_this2._record.parentTable.loadCellValuesInFieldIdsAsync([_this2._field.id]), _this2._linkedQueryResult.loadDataAsync(), _this2._loadRecordColorsAsync()]);

            case 5:
              _this2._invalidateComputedData();
