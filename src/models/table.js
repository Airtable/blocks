...
import type {BaseDataForBlocks, TableDataForBlocks, RecordDataForBlocks} from 'client/blocks/blocks_model_bridge/blocks_model_bridge';
...
class Table extends AbstractModelWithAsyncData<TableDataForBlocks, WatchableTableKey> {
    ...
    async _loadCellValuesInFieldIdsAsync(fieldIds: Array<string>): Promise<Array<WatchableTableKey>> {
        const {recordsById: newRecordsById} = await liveappInterface.fetchAndSubscribeToCellValuesInFieldsAsync(this._id, fieldIds);
    ...
    _unloadCellValuesInFieldIds(fieldIds: Array<string>) {
        liveappInterface.unsubscribeFromCellValuesInFields(this._id, fieldIds);
