...
export function decorateModuleNotFoundError(
    ...
    // captureStackTrace is only available on v8
...
function createFriendlyErrorMessageForModuleNotFound(
    ...
    if (isRelativeImport) {
        // Relative imports are a sign that this is a first party module. It's likely that this is
        // a typo on the import path rather than not having dependencies installed
        return `Please check that the import path "${importPath}" is correct in "${fileContainingError}" at ${locationString}.`;
    }

    // Rudimentary attempt to determine if we should suggest `npm install` or `yarn install`.
    // There might not even be a lockfile yet if the user never installed dependencies!
