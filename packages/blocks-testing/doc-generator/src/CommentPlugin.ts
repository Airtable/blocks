...
import {Comment, CommentTag} from 'typedoc/dist/lib/models/comments/index';
import {IntrinsicType} from 'typedoc/dist/lib/models/types/index';
import {
    Reflection,
    ReflectionFlag,
    ReflectionKind,
...
import {parseComment, getRawComment} from 'typedoc/dist/lib/converter/factories/comment';
...
// Adapted from the TypeAlias converter plugin:
// https://github.com/TypeStrong/typedoc/blob/fe2363c6a1817df177ee7b54fbc8f06f17b240bb/src/lib/converter/plugins/CommentPlugin.ts

/**
 * A handler that parses javadoc comments and attaches [[Models.Comment]] instances to
 * the generated reflections.
 */
@Component({name: 'CommentPlugin'})
export default class CommentPlugin extends ConverterComponent {
    /**
    ...
    private applyModifiers(reflection: Reflection, comment: Comment) {
        ...
        if (
            ...
            (comment.hasTag('internal') &&
                this.application.options.getCompilerOptions().stripInternal)
        ) {
            if (!this.hidden) {
                this.hidden = [];
            }
    ...
    private onDeclaration(context: Context, reflection: Reflection, node?: ts.Node) {
        ...
        if (
            ...
            // This code block was modified in our forked version to support adding
            // comments to functions. See https://github.com/TypeStrong/typedoc/issues/1144
                ...
                switch (property) {
                    ...
                    case TraverseProperty.Parameters:
                        if ((<SignatureReflection>reflection.parent).parameters) {
                            const index = (<SignatureReflection>(
                                reflection.parent
                            )).parameters!.indexOf(<ParameterReflection>reflection);
                            if (index !== -1) {
                                (<SignatureReflection>reflection.parent).parameters!.splice(
                                    index,
                                    1,
                                );
                            }
                    ...
                    case TraverseProperty.Signatures:
                        if (parent.signatures) {
                            const index = parent.signatures.indexOf(
                                <SignatureReflection>reflection,
                            );
                            if (index !== -1) {
                    ...
                    case TraverseProperty.TypeParameter:
                        if (parent.typeParameters) {
                            const index = parent.typeParameters.indexOf(
                                <TypeParameterReflection>reflection,
                            );
                            if (index !== -1) {
