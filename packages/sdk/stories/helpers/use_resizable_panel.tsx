// Ported over from Hyperbase, replacing `u` for lodash.
...
export default function useResizablePanel({
    ...
    useEffect(() => {
        const _updateMaxHeight = rafThrottle(() => {
            // We use the parent node to decide the max height.
    ...
    if (isResizing) {
        // We render an invisible overlay node to prevent pointer events on iframes that will steal the mousemove events.
        // Also this gives us a nice way to deal with mouse events instead of adding them to the `document`.
