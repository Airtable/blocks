...
function SelectableIconWithLabel({
    ...
    useEffect(() => {
        // Rely on the DOM here to scroll the active item in view.
        const {current} = ref;
        if (current && isChecked) {
            ...
            if (currentBottomOffset > parent.offsetHeight + parent.scrollTop) {
                // The item is at the bottom, but not completely in view, scroll it into view.
                parent.scrollTop = currentBottomOffset - parent.offsetHeight + 16;
            } else if (current.offsetTop < parent.scrollTop) {
                // The item is at the top, but not completely in view, scroll it into view.
        ...
        <Example
            options={{
                size: {
                    type: 'selectButtons',
                    label: 'Size',
                    // This needs to be a number for now so the micro icon gets picked automatically.
                    options: [12, 16, 20],
                    defaultValue: 16,
                },
            }}
