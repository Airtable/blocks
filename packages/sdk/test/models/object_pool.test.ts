import ObjectPool, {Poolable} from '../../src/models/object_pool';
...
// Simulating the passage of one minute is expected to ensure that any active
// ObjectPools will release objects to which they have only "weak" references.
...
describe('ObjectPool', () => {
    ...
    it('is capable of tracking weak references to multiple objects for the same key', () => {
        ...
        // Change key so the subsequent retrieval fails, resulting in two
        // objects being tracked for the key "b".
        second.__poolKey = 'definately not b';
        pool.getObjectForReuse('b');

        // Restore key so that the weak reference removal logic recognizes the
        // object.
        second.__poolKey = 'b';
        simulateOneMinute();
        const fourth = pool.getObjectForReuse('b');
    ...
    it('recognizes key changes when creating strong references', () => {
        const first = pool.getObjectForReuse('a');
        // Ensure the pool has a weak reference for an object at another key
        pool.getObjectForReuse('b');
        first.__poolKey = 'b';
