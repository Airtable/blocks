...
This will start a local server and create a tunnel with ngrok.io to make the URL accessible
remotely. If you don't want to use the tunnel (for example, if you're on a slow internet connection)
you can start the server in local mode:

`block run --local`

This will skip setting up the ngrok tunnel and instead serve the bundle over https locally, with a
...
Once the server is started (in both local or ngrok mode) paste the bundle URL into the "Edit block
...
Once you're happy with your changes, use `block push` to upload the code to the block. Pushing will
update the code others users see when they open the web IDE or when they pull.
...
-   If you want to rename the entry module, use `block rename-entry <new name>` instead of renaming
    the file directly (rename-entry will also update the metadata so the block syncs properly the
    next time you push)
