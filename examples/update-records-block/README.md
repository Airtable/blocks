# Update records extension

This example extension updates the cell values of the selected records in your base, adding 1 to the
current cell value of a field in each record.

The code shows:

-   How to use the Cursor API to detect when a user has selected records in grid view, and how to
    get the selected records.

-   How to update multiple records from your extension.

-   How to check if the user has write permissions before performing an update.

## How to run this extension

1. Create a new base using the
   [Menu Planning template](https://airtable.com/templates/event-planning/expvAPRfjIaE5Js68/menu-planning)

2. Create a new extension in your new base (see
   [Create a new extension](https://airtable.com/developers/blocks/guides/hello-world-tutorial#create-a-new-app)),
   selecting "Update records" as your template.

3. From the root of your new extension, run `block run`.

## See the extension running

![Extension with a button that adds 1 to 'In Stock' for each selected record](media/block.gif)
