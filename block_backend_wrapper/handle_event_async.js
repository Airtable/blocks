...
async function uploadLogsToS3Async(logFilePath: string, logStream: fs.WriteStream, presignedS3UploadUrl: string) {
...
function getRouteAndParamsForEvent(event: LambdaEvent): {
    route: Object,
...
async function callUserCodeForEventAsync(event: LambdaEvent): Promise<HandlerResponseObj> {
    ...
    try {
        ...
        if (typeof routeHandlerModule !== 'object' || typeof routeHandlerModule.default !== 'function') {
            throw new Error(`Route module's default export must be a function: ${route.metadata.name}`);
        }
