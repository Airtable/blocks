...
const parse = (input, options = {}) => {
    ...
    while (remaining !== '') {
        ...
        // block comment open
        if (BLOCK_OPEN_REGEX && !(tripleQuotes && block.type === 'block')) {
        ...
        // block comment close
        if (BLOCK_CLOSE_REGEX && block.type === 'block') {
        ...
        if (LINE_REGEX && block.type !== 'block' && options.line) {
            if ((token = scan(LINE_REGEX, 'line'))) {
                ...
                if (
                    WHITE_LISTED_TOKENS.some(
                        whiteListedToken => token.value.indexOf(whiteListedToken) !== -1,
                    )
                ) {
                    token.type = 'text';
                }
